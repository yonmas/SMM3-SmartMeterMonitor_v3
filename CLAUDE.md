...
| --- | --- | --- |
| Files | `smm3_main.py`, `smm3_main_web.py`, `smm3_sub.py`, `smm3_sub_core2.py`, `bp35a1.py`, `func_main.py`, `func_sub.py`, `func_sub_core2.py`, `calc_charge.py`, `calc_charge/`, `lite/*` | `smm3_sub_atoms3.py`, `test_atoms3/*` |
| Devices | M5StickC Plus (parent), M5Stack Basic, M5Stack Core2 | M5Stack ATOM S3 |
...
The "CircuitPython Constraints" section below (f-strings, `decode()`, ESP-NOW argument order, etc.) applies **only** to the right-hand column. Do not apply it when editing `smm3_main.py`, `smm3_sub.py`, `smm3_sub_core2.py`, or anything under `lite/`.
...
| Path | Purpose |
|------|---------|
| `smm3_main_web.py` | **Current parent unit** (UIFlow MicroPython) — smm3_main.py plus GAS dashboard upload, hard WDT, and freeze defenses. This is what runs on the deployed device; see `docs/ARCHITECTURE.md` §5 before touching its send paths |
| `smm3_main.py` | Legacy parent unit without the GAS/Web layer — kept as the simpler reference |
...
| `calc_charge.py` | The charge-calculation module actually imported by `smm3_main.py` (`from calc_charge import CalcCharge`) — see "Modular Charge Calculation" below for how it gets there |
| `calc_charge/` | Source modules for charge calculation, one per power company, not imported directly |
| `lite/` | Alternative main programs (parent + Basic/Core2 children) for meters with short history |
| `gas_dashboard/` | Google Apps Script web app (Code.js, History.js, Dashboard.html) receiving the parent's HTTPS POSTs; History data lives in a Google Sheet. Deploy note: changes need a **new deployment** to affect the `/exec` URL |
...
| `config_files/` | Sample/template JSON config files (`config_main.json`, `config_sub.json`, `api_config.json`); underscore-prefixed versions are blank templates |
| `archives/` | Frozen historical snapshots of old versions — not active code, do not edit |
...
1. **Parent** (`smm3_main.py`) connects to smart meter and:
