...
- **Webダッシュボードの時間別比較表、最下段の合計・比率の計算に不具合があり修正しました**。今日の実績（現時刻まで）と比較対象の平均を、24時間フル分のまま比べてしまっていたため、日中は比率が実態より低く表示されていました。子機の表示と同じく、両方とも現時刻までの範囲で揃えて計算するよう修正しています。あわせて、合計の丸め桁数も表示桁（小数点以下1桁）に統一し、子機の表示値と一致するようにしました。
  **既に`gas_dashboard/`のファイルをコピーしてご自身でデプロイ済みの方は、`Code.js`と`History.js`の2ファイルを最新版に差し替えてください**（`Dashboard.html`・`appsscript.json`は今回変更なし）。
...
- liteバージョンにも Webダッシュボード対応版（`smm3-lite_main_web.py`）を追加しました。使い方は通常版と同じです。
...
- **電力超過の警告**：設定した警告アンペアを超えると、瞬時画面の数値が赤くなります（親機・子機と同じ配色）。
...
- **liteバージョン** の仕様は以下の通りです。
  ...
  - 電気料金は検針日を無視した上記期間の参考値です。料金計算用の **基本料金**、**燃料費調整額**、**再エネ賦課金** は0にしておくとよいでしょう。
