"""
SMM3 ATOM - 4モード切り替え
CircuitPython on ATOM S3R (128×128)
...
【ATOM S3R対応・既知の制約（2026-08-04時点、smm3_sub_atoms3.pyからの分岐）】
S3R無印にほぼ同じコードだが、以下の実機差異(BMI270・別I2Cバス)により
CircuitPythonのbitbangio I2C書き込みが不安定で未解決の問題がある：
  - 画面バックライトが点灯しない（描画自体は正常、SPI経由のため無関係）
    バックライトはIMUと同じI2Cバス(SCL=GPIO0,SDA=GPIO45)上のアドレス0x30の
    専用チップで制御されており、書き込みが通らない
  - 画面自動回転が効かない（IMU=BMI270の初期化に失敗するため、後述の
    _imu_okガードで安全に無効化されるのみ）
詳細な調査記録は project_smm3_atom.md メモリ参照。本格解決（自前ビルドの
CircuitPython or Arduino/PlatformIOへの切替）は保留中。
"""
...
# ---- IMU (MPU6886を前提としたコード。S3RはBMI270で非対応・後述) ----
# board.IMU_SCL/IMU_SDAは無印S3向けのピンで、S3R実機では未接続のため初期化が
# 必ず失敗する(実機確認済み、2026-08-04)。S3RのBMI270対応は本格実装が必要
# (ファイル冒頭のコメント参照、保留中)。それまでの間、初期化失敗時は
# _imu_ok=Falseにしてアプリ全体はクラッシュさせず、回転機能だけを諦める。
_MPU_ADDR   = 0x68
_PWR_MGMT_1 = 0x6B
_ACCEL_XOUT = 0x3B
...
try:
    ...
    print("[IMU] init failed, rotation disabled:", type(e).__name__, e)
