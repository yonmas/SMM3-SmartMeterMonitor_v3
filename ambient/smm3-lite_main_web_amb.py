# SMM3 親機 lite版 Ambient対応版（子機あり）：smm3-lite_main_web.py + Ambient送信。
# smm3-lite_main_web.py との差分：Ambient送信（send_ambient()とその周辺、軽量版ambient_lite.py使用）。
# 子機(ESP-NOW)・GAS(Web)・Ambientを同時に動かす構成のため、メモリ制約によりバックフィル送信の
# 成功率は60〜78%程度に留まる（子機を使わないambient/smm3-lite_main_web_amb_mini.pyなら100%）。
...
# GAS送信先はデプロイID限定とし、フルURL/ID自動判定の分岐（'://' in判定・split()）を無くし、
# 単純な文字列連結だけにする。'+'連結ではなく'%'フォーマットで一時オブジェクトを減らす
# （'+'は中間文字列を1個作ってから最終結果をもう1個作るため2回確保になる）。
# GSS読込みは無し（起動時のconfigファイル読込み1回のみで確定）。
