# SMM3 親機 Ambient対応版（子機なし）：smm3_main_web_amb.py から子機(ESP-NOW)送受信を省いた版。
# 子機(ESP-NOW)とGAS(Web)送信＋Ambient送信を同時に動かすと、ESP-NOWのメモリ占有分だけ
# バックフィル送信の成功率が下がる（実測：子機あり60〜78% → 子機なし100%、2026-08-09検証）。
# 子機を使わない運用（親機単体、または子機以外の方法で電力を確認する運用）であれば、
# ESP-NOW関連の送受信を丸ごと省くことでこの制約を回避できる。詳しくは ambient/README.md 参照。
# smm3_main_web_amb.py との差分は espnow.init()・espnow.broadcast()・espnow.recv_data()と
...
# 撤去し、単純な文字列連結（'%'フォーマット）だけにしている（2026-08-10、バリエーションを
# 揃えるため）。空文字は送信無効のまま。
