class CalcCharge:
    ...
    def tepco(self, contract, hourly_power, day, UNIT):
        ...
        power = int(sum(sum(row) for row in hourly_power) * UNIT)
    ...
    def tepco_smartlife_s(self, contract, hourly_power, day, UNIT):
        ...
        # 時間帯ごとの使用電力量の集計
        power1 = sum(sum(row[i] for i in range1) for row in hourly_power) * UNIT
        power2 = sum(sum(row[i] for i in range2) for row in hourly_power) * UNIT
        power3 = sum(sum(row[i] for i in range3) for row in hourly_power) * UNIT
    ...
    def chubu_smartlife(self, contract, hourly_power, day, UNIT):
        ...
        power3_weekday = 0
        power3_weekend = 0
        power4 = sum(sum(row[i] for i in range4) for row in hourly_power) * UNIT
        power5 = sum(sum(row[i] for i in range5) for row in hourly_power) * UNIT

        for ago_date in range(0, len(hourly_power[0])):
            if (day - ago_date) % 7 < 5:  # 平日
                power3_weekday += sum(hourly_power[ago_date][i] for i in range3) * UNIT
            else:  # 週末
                power3_weekend += sum(hourly_power[ago_date][i] for i in range3) * UNIT
